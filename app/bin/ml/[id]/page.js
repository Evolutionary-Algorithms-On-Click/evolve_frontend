...
export default function MLExecResult() {
    ...
    const [bestContent, setBestContent] = useState(""); // Changed from bestFitness for ML context
    ...
    const stopLogStreaming = () => {
        if (sseAbortControllerRef.current) {
            ...
            // Avoid setting status directly here if fetchData will be called immediately after
            // setSseStatus("closed");
        }
    };
    ...
    const fetchFinalResults = () => {
        ...
        fetchLogsContent();
        fetchBestContent(); // Use the correct function for ML
    };
    ...
    useEffect(() => {
        ...
        if (id) {
            console.log(
                "ML Component Mounted/ID changed. Fetching static data and initial status.",
            );
            fetchInputParams();
            fetchCodeContent();
        ...
        // --- Cleanup Function ---
        return () => {
            console.log("ML Execution component cleanup: Stopping SSE stream.");
            stopLogStreaming();
        };
    ...
    // --- Fetch Final Best Content (e.g., accuracy, model details) ---
        ...
        <main className="flex flex-col items-center justify-center min-h-screen font-[family-name:var(--font-geist-mono)] p-4 sm:p-8 bg-gray-100">
            ...
            <div className="text-center mb-6">
                <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">
                    Evolve OnClick - ML Run
                </h1>
                <p className="text-gray-600">
                    Visualize Machine Learning Evolution.
                </p>
            </div>
            ...
            <div className="flex flex-col lg:flex-row mt-6 gap-4 w-full max-w-7xl">
                {/* ML Input Parameters Preview */}
                {inputParams && (
                    <div className="border border-gray-300 rounded-xl bg-white bg-opacity-90 shadow-sm p-0 overflow-hidden max-w-[34%] flex-shrink-0 h-fit">
                        {/* Use PreviewML component */}
                        <PreviewML
                            datasetURL={inputParams.googleDriveUrl ?? ""}
                            targetColumnName={
                                inputParams.targetColumnName ?? ""
                            }
                            sep={inputParams.sep ?? ","}
                            mlImportCodeString={
                                inputParams.mlImportCodeString ?? ""
                            }
                            mlEvalFunctionCodeString={
                                inputParams.mlEvalFunctionCodeString ?? ""
                            }
                            chosenAlgo={inputParams.algorithm ?? "N/A"}
                            mu={inputParams.mu ?? 0}
                            lambda={inputParams.lambda ?? 0}
                        ...
                        <div className="w-full">
                            ...
                            {bestContent && (
                                <div className="mb-4">
                                    ...
                                    {/* Display best ML result (e.g., accuracy, model params) */}
                                    <pre className="rounded-lg text-sm mt-2 overflow-auto bg-gray-100 p-3 whitespace-pre-wrap break-words">
                                        <code>{bestContent}</code>
                                    </pre>
                                </div>
                            )}
                            {/* ML Plots (adjust filenames/content as needed) */}
                            <div className="mt-4 w-full">
                                <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-2">
                                    Plots
                                </h3>
                                {/* Fitness/Accuracy Plot */}
                                <div className="mb-4">
                                    <h5 className="text-sm text-gray-600 mb-1">
                                        Fitness/Accuracy Plot
                                    </h5>
                                    <Image
                                        src={`${minioBaseUrl}/code/${id}/fitness_plot.png`} // Common name, verify if correct for ML
                                        alt="Fitness or Accuracy Plot"
                                        width={700}
                                        height={400}
