    ...
    return (
        <div className="mt-16">
            ...
            {indGen && ["floatingPoint", "integer"].includes(indGen) && (
                <div className="mt-4">
                    <h5 className="text-lg font-bold">Step 3.1: Random Range</h5>
                    {(randomRangeStart && randomRangeEnd && (
                        isNaN(randomRangeEnd) || isNaN(randomRangeStart) || parseInt(randomRangeStart) > parseInt(randomRangeEnd)
                    )) && (
                            <p className="text-blue-500 mb-3">
                                Selected range is invalid. Please make sure the start is less than the end and both are numbers.
                            </p>
                        )}
                    <div className="flex gap-4">
                        <input type="number" className="border border-gray-300 p-2 rounded-lg" placeholder="Start" value={randomRangeStart.toString()} onChange={(e) => {
                            setRandomRangeStart(e.target.value)

                            if (randomRangeEnd && e.target.value) {
                                if (isNaN(randomRangeEnd) || isNaN(e.target.value) || parseInt(randomRangeEnd) < parseInt(e.target.value)) {
                                    return;
                                }
                        ...
                        <input type="number" className="border border-gray-300 p-2 rounded-lg" placeholder="End" value={randomRangeEnd.toString()} onChange={(e) => {
                            setRandomRangeEnd(e.target.value)

                            if (randomRangeStart && e.target.value) {
                                if (isNaN(randomRangeStart) || isNaN(e.target.value) || parseInt(randomRangeStart) > parseInt(e.target.value)) {
                                    return;
                                }
